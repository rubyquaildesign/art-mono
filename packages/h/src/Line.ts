...
export class Line {
  ...
  intersectionPoint(other: Line): IntersectionResult {
    ...
    const p = this.start.clone();
    const q = other.start.clone();
    const diffVec = q.clone().sub(p);
    // Co-linear case
    if (r.cross(s) === 0 && q.clone().sub(p).cross(r) === 0) {
      // Lines are colinear
    ...
    if (r.cross(s) && fractional(t) && fractional(u))
      return {
        kind: IntersectionType.intersect,
        intersects: true,
        pt: p.clone().add(r.clone().mulScaler(t)),
        tFromA: t,
      };
  ...
  projectPointToLine(point: Vec): ProjectionResult {
    ...
    const pStart = point.clone().sub(this.start);
    const t = pStart.dot(this.vector) / l2;
    const onLine = fractional(t);
    return { onLine, t, pt: this.vector.mulScaler(t).add(this.start) };
  }
