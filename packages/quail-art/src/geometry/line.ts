...
type IntersectionType = typeof IntersectionType;
type Overlap = {
...
export class Line {
	...
	intersectionPoint(other: Line): IntersectionResult {
		...
		if (r.cross(s) === 0 && q.sub(p).cross(r) === 0) {
			...
			if (s.dot(r) < 0) {
				if (t1 > 1 || t0 < 0) return { kind: IntersectionType.parallel, intersects: false };
			...
			if (t0 > 1 || t1 < 0) return { kind: IntersectionType.parallel, intersects: false };
