...
export class Matrix extends Array<number[]> {
	...
	setPos(newValue: number, col: number, row: number) {
		if (col >= this.numCols) throw new Error(`col ${col} is larger then matrix size of ${this.numCols}`);
		if (row >= this.numRows) throw new Error(`col ${row} is larger then matrix size of ${this.numRows}`);
